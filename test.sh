...
if [[ "$(docker images -q megabytelabs/DOCKERFILE_SLUG:slim 2> /dev/null)" == "" ]] || [[ "$(docker images -q megabytelabs/DOCKERFILE_SLUG:latest 2> /dev/null)" == "" ]]; then
...
for TEST_SCENARIO in test/*/; do
  ...
  LATEST_OUTPUT=$(docker run -v "${PWD}:/work/${TEST_SCENARIO}" -w /work megabytelabs/DOCKERFILE_SLUG:latest DOCKERFILE_COMMAND)
  log "Running the slim image and capturing the output for the ${TEST_SCENARIO} scenario"
  SLIM_OUTPUT=$(docker run -v "${PWD}:/work/${TEST_SCENARIO}" -w /work megabytelabs/DOCKERFILE_SLUG:slim DOCKERFILE_COMMAND)
